#!/usr/bin/env node
/**
 * HTML to PNG Generator for PeakPack Ad Variants
 * Creates actual PNG files by rendering HTML/CSS versions of each variant
 */
...
// Generate HTML template for each variant
function generateHTML(variant) {
  ...
  let headlineSize = isVertical ? '42px' : isSquare ? '46px' : '56px';
  let descriptionSize = isVertical ? '16px' : isSquare ? '18px' : '22px';
  let priceSize = isVertical ? '16px' : isSquare ? '18px' : '20px';
  let ctaSize = isVertical ? '16px' : isSquare ? '18px' : '20px';
...
async function generateAllHTML() {
  try {
    console.log("🚀 Generating HTML files for 27 PeakPack ad variants...");
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PeakPack Ad Variants - Gallery</title>
    <style>
...
<body>
    <h1>PeakPack Ad Variants (${variants.length} total)</h1>
    <div class="gallery">
    ...
    console.log(`\n✅ Successfully generated ${variants.length} HTML ad variants!`);
    console.log(`📁 Files saved to: ${htmlDir}`);
    console.log(`📋 Manifest: manifest.json`);
    console.log(`🖼️ Gallery: index.html`);
    console.log(`\n🔗 To convert HTML to PNG, you can use:`);
    console.log(`   - Puppeteer (headless Chrome)`);
    console.log(`   - wkhtmltopdf`);
    console.log(`   - Online HTML to PNG converters`);
...
module.exports = { generateAllHTML, generateHTML, variants };